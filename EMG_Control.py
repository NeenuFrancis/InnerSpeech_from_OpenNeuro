...
def EMG_control_single_th(root_dir: str, N_Subj_arr: list, N_block_arr: list,
                          low_f: float, high_f: float, t_min: float,
                          t_max: float, window_len: float, window_step: float,
                          std_times: Union[int, float], t_min_baseline: float,
                          t_max_baseline: float, tag_events: bool = False,
...
# =============================================================================
            ...
            mean_Base_energy_7, std_Base_energy_7 = calculate_power_windowed(Channel_filter, FC, window_len,        # noqa
                                                                             window_step, t_min_baseline,           # noqa
                                                                             t_max_baseline)                        # noqa
# =============================================================================
            # EXG7 Control
            Baseline = X_baseline.copy()
            ...
            mean_Base_energy_8, std_Base_energy_8 = calculate_power_windowed(Channel_filter, FC, window_len,        # noqa
                                                                             window_step, t_min_baseline,           # noqa
                                                                             t_max_baseline)                        # noqa
...
# =============================================================================
            ...
            for n_trial in range(Trials):
                ...
                mean_Trial_Energy_7 , _ = calculate_power_windowed(Channel_filter, FC, window_len,      # noqa
                                                                   window_step, t_min, t_max)           # noqa

                # Get trial
                Channel_filter = EMG_data_EXG8[n_trial, 0, :]
                # Calculate power for channel EXG8
                mean_Trial_Energy_8, _ = calculate_power_windowed(Channel_filter, FC, window_len,       # noqa
                                                                  window_step,
                                                                  t_min, t_max)

                # Threshold condition
                if (mean_Trial_Energy_8 > threshold_8 or mean_Trial_Energy_7 > threshold_7):            # noqa

                    # mark only if is not Pronounced trials
                    if not (Y[n_trial, 2] == Pronunced_id):

                        trial_power_7 = np.append(trial_power_7,
                                                  mean_Trial_Energy_7)
                        trial_power_8 = np.append(trial_power_8,
                                                  mean_Trial_Energy_8)
            ...
            print('Tagged Trials: '+str(len(R_EMG)), 'for Subject ' + str(N_S) + ' in Session '+str(N_B))       # noqa
            ...
            # Save report
            file_name = root_dir + 'derivatives/' + Num_s + '/ses-0' + str(N_B) + '/' + Num_s + '_ses-0'+str(N_B)+'_report.pkl' # noqa
